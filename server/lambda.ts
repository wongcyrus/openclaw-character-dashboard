...
import {
  ...
  type ResolvedAgentCoreRuntimeTarget,
...
const AGENTCORE_RUNTIME_ENDPOINT_ID =
  process.env.AGENTCORE_RUNTIME_ENDPOINT_ID || "";
const AGENTCORE_RUNTIME_NAME = process.env.AGENTCORE_RUNTIME_NAME || "";
const AGENTCORE_RUNTIME_ENDPOINT_NAME =
...
let agentCoreTargetPromise: Promise<ResolvedAgentCoreRuntimeTarget> | null =
  null;
  ...
  if (path === "/api/files") {
    ...
    try {
      ...
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ path: relPath, entries }),
      };
    } catch (err: any) {
      return {
        statusCode: 500,
...
async function fetchAgentCoreSnapshot(): Promise<any> {
  ...
  if (payload.status !== "ready" || !payload.snapshot) {
    throw new Error(
      payload.error ??
        `AgentCore snapshot failed with status ${payload.status}`,
    );
  }
...
async function invokeAgentCoreAction(
  runtimeTarget: ResolvedAgentCoreRuntimeTarget,
  runtimeSessionId: string,
  actionPayload: Record<string, unknown>,
): Promise<any> {
  const client = new BedrockAgentCoreClient({ region: AGENTCORE_REGION });
  const command = new InvokeAgentRuntimeCommand({
    agentRuntimeArn: runtimeTarget.runtimeArn,
...
function getAgentCoreRuntimeTarget(): Promise<ResolvedAgentCoreRuntimeTarget> {
  agentCoreTargetPromise ??= resolveAgentCoreRuntimeTarget({
    region: AGENTCORE_REGION,
    runtimeName: AGENTCORE_RUNTIME_NAME,
    endpointName: AGENTCORE_RUNTIME_ENDPOINT_NAME,
    runtimeArn: AGENTCORE_RUNTIME_ARN,
    qualifier: AGENTCORE_RUNTIME_ENDPOINT_ID,
  });

  return agentCoreTargetPromise;
}

async function resolveAgentCoreIdentity(): Promise<{
  userId: string;
  runtimeSessionId: string;
}> {
