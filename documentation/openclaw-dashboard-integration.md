...
## Why the AgentCore path is a workaround

The AgentCore integration is intentionally an **adapter**, not a pure end-to-end native dashboard protocol.

The dashboard originally expected to talk to a reachable OpenClaw gateway WebSocket. That works in local mode, but it does **not** map cleanly to AgentCore because:

1. the OpenClaw gateway inside the runtime is bound to the runtime container, not directly reachable from the browser
2. the browser cannot safely or directly follow per-user AgentCore session placement
3. Telegram/chat traffic may be answered by either the full OpenClaw gateway **or** the lightweight warm-up/fallback path
...
- **Real events** come from the internal OpenClaw gateway event stream.
- **Synthetic events** are generated by the runtime when a response came from the lightweight warm-up/fallback path, so the dashboard still sees an `agent-message` even though no gateway event existed.

The browser does not need to distinguish them for rendering, but operators should know both sources exist.

### Important limitations

- Event buffering is **in-memory only** inside the runtime session. A runtime restart or new session clears it.
...
```text
                                  ...
                                  └─────────────┬─────────────┘
                                                │
                                                │ InvokeAgentRuntime(action=chat)
                                                ▼
        ┌──────────────────────────────────────────────────────────────────────────┐
        │                    AgentCore Runtime Session                            │
        │                                                                          │
        │  ┌──────────────────────────────┐      ┌──────────────────────────────┐  │
        │  │   agentcore-contract.js      │      │   OpenClaw gateway :18789    │  │
        │  │                              │─────▶│   real chat / agent events    │  │
        │  │  - dashboard_snapshot        │      └──────────────────────────────┘  │
        │  │  - dashboard_events          │                    ▲                    │
        ...
        │      │   early replies before full  │   synthetic dashboard events        │
        │      │   OpenClaw is ready          │                                     │
        │      └──────────────────────────────┘                                     │
        └──────────────────────────────────────────────────────────────────────────┘
...
│                                                                              │
│  snapshot path: InvokeAgentRuntime(action=dashboard_snapshot)                │
│  live path:     poll InvokeAgentRuntime(action=dashboard_events, since=seq)  │
│                 then rebroadcast normalized events to browser WS clients      │
└──────────────────────────────────────────────────────────────────────────────┘
```

### Reading the diagram

1. **Telegram and the dashboard do not talk to each other directly.** They only match because both are pinned to the same AgentCore user/session identity.
2. **The browser never reaches the internal OpenClaw gateway directly.** It only talks to the local Express relay.
...
| `AGENTCORE_USER_ID`            | Optional explicit user ID override. Normally auto-resolved from `CHANNEL#<actorId> / PROFILE`. |
| `AGENTCORE_RUNTIME_SESSION_ID` | Optional explicit runtime session ID override. Normally auto-resolved from `USER#<userId> / SESSION`. |
...
- attaching a derived `state` field (`working` / `idle`) when it can infer one from the event type
- emitting a separate `agent-status` event whenever that derived state changes

**Important:** this does **not** mean the frontend has fully switched to WS-driven state. Today:

- `/api/ws` is the source of **message bubbles** and normalized live agent events
- `/api/openclaw/snapshot` is still the source of truth for the character machine's `working` / `idle` state in `src/data/live.ts`
...
3. the runtime translates internal gateway events and lightweight-path responses into one normalized event stream
...
For the relay to show the same conversation as Telegram, these values must describe the **same user/session**:
...
- `AGENTCORE_ACTOR_ID` should match the Telegram sender identity, e.g. `telegram:228465334`
- `AGENTCORE_USER_ID` should match the internal user record resolved by the router
- `AGENTCORE_RUNTIME_SESSION_ID` should match the current session stored for that user

If Telegram replies work but the dashboard sees no events, stale session pinning is one of the first things to check.

### Lightweight-path behavior

During warm-up, crash recovery, or certain fallback cases, a user request can be answered before the full OpenClaw gateway produces a normal chat event. In that case the runtime now emits synthetic dashboard events so the web UI still shows the same response that Telegram received.

Without that normalization step, Telegram could reply successfully while `/api/ws` stayed silent.
...
This is the **current authoritative path** for movement/status. The relay may attach derived `state` metadata to WS events, but the frontend has not fully replaced snapshot-based derivation with those WS hints yet.
